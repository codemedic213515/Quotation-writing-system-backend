...
public class QuotationService
{
    ...
    public async Task<object> CalculateQuotationCost(string quotationNumber)
    {
        ...
        decimal overheadRate = (decimal)(rankMaster.SiteMiscell ?? 0) / 100;
        decimal miscellaneousRate = (decimal)(rankMaster.OtherExpens ?? 0) / 100;
        ...
        try
        {
            ...
            foreach (var quotationType in quotationTypes)
            {
                ...
                var materialDetails = materials.Select(m => new
                {
                    ...
                    Quantity = Convert.ToDecimal(m.Quantity),
                    Price = Convert.ToDecimal(m.Price),
                    Amount = Convert.ToDecimal(m.Quantity) * Convert.ToDecimal(m.Price),
                    StepRate = Convert.ToDecimal(m.StepRate)
                }).ToList();
